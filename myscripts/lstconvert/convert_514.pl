...
print LFH "PCGen 5.12 to 5.14 conversion program v0.85\n";
print LFH "Copyright (c) 2008 Chris 'Barak' Chandler\n";
print LFH "-----------------------------------------\n";
print LFH scalar localtime() . "\n\n";
foreach (@ARGV) {
    if ( $_ =~ /-S/ ) {
        open( SFH, "> shieldprof.lst" )
          || die "Can't create shieldprof file: $!";
        print SFH "\nBuckler\nShield (Light)\nShield (Heavy)\nTower Shield";
...
#####
    if ( -d $_ ) {
        if ( $_ =~ /^\./ ) {
            ...
            $out_path =
              $output_dir . substr( $File::Find::name, length($input_dir) );
        }
    }
#####
# Process .pcc file
...
#####
     ...
     #convert SA and any PRExxx that need it and are attached to the SA
                    else {
                        $field =~ s/^SA:/SAB:/g;
                        ...
                        $field =~ s/PREEQUIP:([A-Za-z])/PREEQUIP:1,$1/g;
                        print OFH $field;
                        if ( $fieldcount <= $#fields ) {
                            print OFH "\t";
                        }
...
#   -- single instance
#####
                elsif (    $field =~ /PRESPELLSCHOOLSUB:[A-Za-z]/
                        && $field !~ /\[/ )
                {
                    $field = substr( $field, rindex( $field, ":" ) + 1 );
                    @workarray = split( ",", $field );
                    print OFH "PRESPELLSCHOOLSUB:"
                      . $workarray[1] . ","
                      . $workarray[0] . "="
                      . $workarray[2] . "\t";
                }
...
#   -- single instance
#####
                elsif (    $field =~ /PRESPELLDESCRIPTOR:[A-Za-z]/
                        && $field !~ /\[/ )
                {
                    $field = substr( $field, rindex( $field, ":" ) + 1 );
                    @workarray = split( ",", $field );
                    print OFH "PRESPELLDESCRIPTOR:"
                      . $workarray[1] . ","
                      . $workarray[0] . "="
                      . $workarray[2] . "\t";
                }
...
#                    }
                    # split TYPE.Arcane,TYPE.Divine in SPELLLEVEL
                        ...
                        $class1 =
                          substr( $workarray[1], 0,
                                  index( $workarray[1], "," ) );
                        $class2 = substr( $workarray[1],
                                          index( $workarray[1], "," ) + 1 );
                        $levelend =
                          substr( $workarray[3], 0,
                                  index( $workarray[3], "[" ) );
                        print OFH "CHOOSE:SPELLLEVEL\|"
                          . $workarray[0] . "\|"
                          . $class1 . "\|"
                          . $workarray[2] . "\|"
                          . $levelend . "\|"
...
# remove invalid LIST parameter (only seems to occur in those that have the trailing #)
                        ...
                        if ( $field =~ /\|LIST/ ) {
                            $haslist = 1;
                            $field =
                              substr( $field, 0, rindex( $field, "\|" ) );
                        }
                        if ( $field =~ /NONCLASSSKILLLIST/ ) {
                            $field =
                              "CHOOSE:SKILLSNAMED|CROSSCLASS|EXCLUSIVE\t";
                        }
                  ...
                  # if it is one of the documented subtokens, pass it on through
                        ...
                        # if it is not a documented subtoken, use CHOOSE:STRING
                            ...
                            if ( $field =~ /CHOOSE:NUMCHOICES/ ) {
                                ...
                                $Temp = substr( $field, index( $field, "\|" ) );
                                $Temp =
                                  substr( $Temp, index( $Temp, "\|" ) + 1 );
                                print OFH "\|STRING\|";
                                print OFH $Temp . "\t";
                            }
                            else {
                                print OFH "CHOOSE:STRING|"
                                  . substr( $field, rindex( $field, ":" ) + 1 )
                                  . "\t";
                            }
                        }
...
#####
                    $field =~ s/PRESA:([A-Za-z])/PRESA:1,$1/g;
#####
# PREEQUIP to PREEQUIP:#,y,y conversion
#####
                    $field =~ s/PREEQUIP:([A-Za-z])/PREEQUIP:1,$1/g;
#####
...
#####
    if ( $#armorprofs > 0 ) {
        ...
        open( AFH, "> $armorprof_file" )
          || die "Can't create armorprof file: $!";
        foreach $armorprof (@armorprofs) {
            print AFH $armorprof . "\n";
        }
        if (    $base_name =~ "phb_d20_fantasy_v35e"
             || $base_name =~ "aesrd_ogl_fantasy"
             || $base_name =~ "ausrd_ogl_fantasy"
             || $base_name =~ "rsrd_d20_fantasy_v35e"
             || $base_name =~ "srd_d20_fantasy_v30e" )
        {
